...
getData(`/admin/subscription_type?type=QR_CODE`, METHODS.GET).then((response) => {
    if (response.success) {
...
function getCards(value) {
    ...
    let query = '?type=QR_CODE';

    if (value) {
        query += `&organization=${value.organization?.id}`;
    }
    ...
    if (cardId) {
        ...
        getData(`/admin/nfc_tag${query}`, METHODS.GET).then((response) => {
            if (response.success) {
