...
watch(searchText, () => {
  getData(`/admin/link?search=${searchText.value}`, METHODS.GET)
      .then((response) => {
...
function getLinks() {
  getData('/admin/link', METHODS.GET)
      .then((response) => {
        if (response.success) {
          linksArr.value = response.result.items;
          pagination.value = {
...
function onRequest(props) {
  ...
  getData(
      `/admin/link?page=${props.pagination.page}&perPage=${props.pagination.rowsPerPage}&${sortQuery}`,
      METHODS.GET,
  )
      .then((response) => {
        linksArr.value = response.result.items;
...
<template>
  <div class="q-pa-lg">
    ...
    <div class="row justify-between items-center page-header">
      <h2 class="text-h4 text-bold q-my-none">Ссылки</h2>
  ...
  >
    ...
    <template v-slot:pagination>
      <q-pagination
        ...
        :max-pages="6"
        @update:model-value="getLinks(`page=${pagination.page}&perPage=14`)"
        direction-links
        outline
        color="black"
