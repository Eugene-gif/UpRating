...
const columns = [
  ...
  // },
  {
    ...
    align: "left",
    field: 'partner',
    format: (val) => `${val ? val.organizationPartner?.title : 'Нет данных'}`,
  },
  {
    name: "users",
    required: true,
...
function getOrganizations(query) {
  loading.value = true;
  getData(`/admin/organization?${query}`, METHODS.GET)
      .then((response) => {
        if (response.success) {
          organizationList.value = response.result.items;
          pagination.value = {
...
<template>
  <div class="q-pa-lg">
    ...
    <div class="row justify-between items-center page-header">
      <h2 class="text-h4 text-bold q-my-none">Клиенты</h2>
      ...
      <div class="row q-gutter-md justify-end">
        ...
        <q-btn
          ...
          outline
          class="btn-default"
          label="Отчёт по регистрациям"
          @click="$router.push(`/organization/report`)"
          target="_blank"
          size="12px"
        />
      </div>
