...
function getLinks() {
    getData(`/admin/link?nfcTag=${ID2}`, METHODS.GET)
        .then((response) => {
            if (response.success) {
...
function onRequest(props) {
    ...
    getData(
        `/admin/link?page=${props.pagination.page}&perPage=${props.pagination.rowsPerPage}&nfcTag=${ID2}&${sortQuery}`,
        METHODS.GET,
    )
        .then((response) => {
            linksArr.value = response.result.items;
                ...
                >
                    <q-list>
                        <q-item
                                clickable
                                class="items-center"
                                @click="$router.push(`/organization/${ID}/qr_codes/${ID2}/link/${props.row.id}`)"
                        >
                            <q-icon
                                    name="info"
                                    size="20px"
