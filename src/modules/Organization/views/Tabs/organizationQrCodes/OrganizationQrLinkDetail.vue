...
if (ID3 != 0) {
    getData(`/admin/link/${ID3}`, METHODS.GET).then((response) => {
        if (response.success) {
...
function saveItem() {
    ...
    getData(isNew ? '/admin/link' : `/admin/link/${ID3}`,
        ...
        .then((response) => {
            if (response.success) {
                ...
                router.push(`/organization/${ID}/qr_codes/${ID2}/links`);
            } else {
                $q.notify({
...
function deleteItem() {
    getData(`/admin/link/${ID3}`, METHODS.DELETE).then((res) => {
        if (res.success) {
            router.push(`/organization/${ID}/qr_codes/${ID2}/links`);
        }
    });
        ...
        <div class="row justify-between items-center page-header">
            ...
            <div class="row q-gutter-md justify-end">
                ...
                <q-btn
                        ...
                        size="12px"
                        @click="$router.push(`/organization/${ID}/qr_codes/${ID2}/links`)"
                />
            </div>
        </div>
    </div>
