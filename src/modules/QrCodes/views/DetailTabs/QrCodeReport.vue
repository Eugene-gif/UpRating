...
function sendReport(byEmail, id) {
    ...
    getData(`/admin/doc/redirect_report_unique?startDate=${startDate.value}&endDate=${endDate.value}&cardId=${id}&send_by_email=true&type=QR_CODE`,
        ...
        emailReportLoading.value = false;
    });
}
...
function downloadReport(id, fileName) {
    ...
    fetch(`${import.meta.env.VITE_API_URL}/admin/doc/redirect_report_unique?startDate=${startDate.value}&endDate=${endDate.value}&cardId=${id}&type=QR_CODE`, {
        method: 'GET',
        headers: {
...
function getReport(byEmail, id) {
    ...
    getData(`/admin/doc/redirect_report_unique?startDate=${startDate.value}&endDate=${endDate.value}&cardId=${id}&renderWeb=true&type=QR_CODE`,
        METHODS.GET)
        .then((response) => {
...
<template>
    <div class="q-pa-lg">
        ...
        <div class="row justify-between items-center page-header">
            <h2 class="text-h4 text-bold q-my-none">Формирование отчета по QR коду</h2>
            ...
            <div class="column q-gutter-md">
                ...
                <div>
                    <q-btn
                            ...
                            :disable="startDate && endDate === ''"
                            label="Скачать отчёт"
                            @click="downloadReport(ID, 'Отчёт по карте')"
                    />
                </div>
