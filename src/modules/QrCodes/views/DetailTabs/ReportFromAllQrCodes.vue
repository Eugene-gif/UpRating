...
const crumbList = [
    ...
    {
        title: 'Формирование отчёта по QR кодам',
...
function sendReport(byEmail, id) {
    ...
    getData(`/admin/doc/redirect_report?startDate=${startDate.value}&endDate=${endDate.value}&organization=${id}&send_by_email=true&type=QR_CODE`,
...
function downloadReport(id, fileName) {
    ...
    fetch(`${import.meta.env.VITE_API_URL}/admin/doc/redirect_report?startDate=${startDate.value}&endDate=${endDate.value}&organization=${id}&type=QR_CODE`, {
        method: 'GET',
        headers: {
...
function getReport(byEmail, id) {
    ...
    getData(`/admin/doc/redirect_report?startDate=${startDate.value}&endDate=${endDate.value}&organization=${id}&renderWeb=true&type=QR_CODE`,
        METHODS.GET)
        .then((response) => {
