...
const columns = [
  ...
  },
  {
    ...
    label: "Партнёр",
    align: "left",
    field: (row) => row.partner.organizationPartner.title,
    format: (val) => `${val ?? 'Нет данных'}`,
    sortable: true,
  },
  {
...
function deleteSettlement(id) {
  confirmDelete('Удалить', (fn) => {
    ...
    getData(`/admin/settlements/${id}`, METHODS.DELETE)
        .then((response) => {
...
<template>
  <div class="q-pa-lg">
    ...
    <div class="row justify-between items-center page-header">
      <h2 class="text-h4 text-bold q-my-none">Взаиморасчёты</h2>
      <div class="row q-gutter-md justify-end">
        <q-btn
          ...
          label="Добавить"
          size="12px"
          @click="$router.push('/settlements/0')"
        />
      </div>
    </div>
  </div>
